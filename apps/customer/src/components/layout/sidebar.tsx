  ...
  return (
    <aside
      className={`
        ...
        ${isMobile ? "w-full" : collapsed ? "w-[64px] md:w-[64px]" : "w-[64px] md:w-[64px] lg:w-[260px]"}
      `}
    >
      {/* Header */}
      ...
      {/* Collapse toggle when collapsed - below header - desktop only */}
      {!isMobile && collapsed && (
      ...
      <div className="mt-auto border-t border-border/50">
        ...
        {mounted ? (
          <div
            className={`flex items-center px-2 py-2 ${isMobile || !collapsed ? "gap-1" : "justify-center"}`}
          >
            {!isMobile && collapsed ? (
              <button
                onClick={() =>
              ...
              <div className={`flex w-full bg-muted/50 rounded-lg p-1 gap-1 ${isMobile ? "" : "md:hidden lg:flex"}`}>
                <button
                ...
                >
                  ...
                  <span className={isMobile ? "" : "md:hidden lg:inline"}>Light</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className={isMobile ? "" : "md:hidden lg:inline"}>Dark</span>
                </button>
                <button
                ...
                >
                  ...
                  <span className={isMobile ? "" : "md:hidden lg:inline"}>System</span>
                </button>
              </div>
            )}
            {!isMobile && collapsed && (
              <button
                ...
                className="text-muted-foreground hover:text-foreground p-1.5 rounded-md hover:bg-accent/50 transition-colors md:flex lg:hidden"
                title={`Theme: ${theme}`}
              >
                {createElement(themeIcon, { className: "w-4 h-4" })}
              </button>
        ...
        {/* Avatar / Profile Row */}
        <div
          className={`flex items-center px-3 py-3 ${isMobile || !collapsed ? "gap-3" : "justify-center"} cursor-pointer hover:bg-accent/30 transition-colors`}
          ...
          {(isMobile || !collapsed) && (
            <>
              ...
              <button
                ...
                className={`text-muted-foreground hover:text-foreground p-1.5 rounded-md hover:bg-accent/50 transition-colors flex-shrink-0 ${isMobile ? "" : "md:hidden lg:block"}`}
              >
...
function NavItem({ href, icon: Icon, label, active, collapsed, isMobile = false, showAlert }: NavItemProps) {
  ...
  return (
    ...
    >
      ...
      {showLabel && (
        <span
          className={`whitespace-nowrap overflow-hidden ${isMobile ? "" : "md:hidden lg:inline"}`}
          style={
            active
              ? {

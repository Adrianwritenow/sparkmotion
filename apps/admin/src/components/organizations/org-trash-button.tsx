...
export function OrgTrashButton() {
  ...
  const restoreMutation = trpc.organizations.restore.useMutation({
    onSuccess: () => {
  ...
  const restoreAllMutation = trpc.organizations.restoreAll.useMutation({
    onSuccess: (result) => {
      ...
      utils.organizations.list.invalidate();
      toast.success(
        `Restored ${result.restored} organization${result.restored !== 1 ? "s" : ""}`
      );
    },
  });
  ...
  return (
    <>
      <button
        ...
        aria-label="View deleted organizations"
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md flex flex-col">
          <SheetHeader className="pb-4 border-b shrink-0">
            <SheetTitle>Deleted Organizations</SheetTitle>
            <SheetDescription>
          ...
          <div className="flex-1 overflow-y-auto py-4">
            {deletedOrgs.length > 0 && (
              <div className="mb-4">
                ...
                >
                  ...
                  Restore All ({deletedOrgs.length})
                </Button>
              </div>
            )}
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-muted-foreground">No deleted organizations</p>
              </div>
            ) : (
              <div className="space-y-2">
                {deletedOrgs.map((org) => {
                  const remaining = daysRemaining(org.deletedAt!);
                  return (
                    <div
                      key={org.id}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Deleted by {org.deletedByName ?? "unknown"} &middot;{" "}

...
export function EventTrashButton() {
  ...
  const softDeleteMutation = trpc.events.delete.useMutation({
    onSuccess: () => {
      utils.events.trashCount.invalidate();
      utils.events.listDeleted.invalidate();
      utils.events.list.invalidate();
  ...
  return (
    <>
      <button
        ...
        aria-label="View deleted events"
      ...
      <Sheet open={open} onOpenChange={setOpen}>
        <SheetContent side="right" className="w-full sm:max-w-md flex flex-col">
          <SheetHeader className="pb-4 border-b shrink-0">
            <SheetTitle>Deleted Events</SheetTitle>
            <SheetDescription>
            ...
            {deletedEvents.length > 0 && (
              <div className="mb-4">
                ...
                >
                  ...
                  Restore All ({deletedEvents.length})
                </Button>
              </div>
            )}
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                {deletedEvents.map((event) => {
                  const remaining = daysRemaining(event.deletedAt!);
                  return (
                    <div
                      key={event.id}
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          Deleted by {event.deletedByName ?? "unknown"} &middot;{" "}

...
const EVENT_ID = "loadtest-event-1";
...
// ─── Scenario ─────────────────────────────────────────────────────────────────
//   667 pipeline req/sec for 2 minutes = ~6K Redis commands/sec
...
// ─── Test Logic ───────────────────────────────────────────────────────────────
// Reproduces the exact 8-command pipeline from worker.ts + PUBLISH
export default function () {
  ...
  const commands = [
    ...
    ["PFCOUNT", `analytics:${EVENT_ID}:taps:unique`],
  ...
  // Separate PUBLISH call (matches worker.ts)
  if (ok) {
    const publishBody = JSON.stringify([
      ["PUBLISH", `tap-updates:${EVENT_ID}`, JSON.stringify({
        totalTaps: 0,
        uniqueTaps: 0,
        mode,
      })],
    ...
    http.post(`${UPSTASH_URL}/pipeline`, publishBody, {
      headers: {
        Authorization: `Bearer ${UPSTASH_TOKEN}`,
        "Content-Type": "application/json",
      },

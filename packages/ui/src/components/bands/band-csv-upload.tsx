...
interface BandCsvUploadDialogProps {
  onUpload: (bandIds: string[]) => Promise<{ created: number }>;
  onAfterUpload?: () => void;
}

export function BandCsvUploadDialog({ onUpload, onAfterUpload }: BandCsvUploadDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    setIsUploading(true);
    try {
      const res = await onUpload(bandIds);
  ...
  const previewContent = (
    <div className="space-y-4">
      ...
      <div className="flex gap-2">
        <Button onClick={handleConfirm} disabled={validRows.length === 0 || isUploading}>
          {isUploading ? "Importing..." : `Confirm Import (${validRows.length} bands)`}
        </Button>

...
interface KpiCardsProps {
  ...
  eventId?: string;
  orgId?: string;
  /** Admin-only: show extra KPIs (Post-Event Taps, Mode Split). Defaults to false. */
...
export function KpiCards({ from, to, eventId, orgId, showAdminKpis = false }: KpiCardsProps) {
  const skeletonCount = showAdminKpis ? 9 : 7;
    ...
    <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
      {kpis.map((kpi) => (
        <Card key={kpi.title}>
          ...
          <CardContent>
            {kpi.format === "custom" && "render" in kpi && kpi.render ? (
              kpi.render()
            ) : (

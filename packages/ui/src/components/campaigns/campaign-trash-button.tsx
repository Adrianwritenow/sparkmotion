...
interface CampaignTrashButtonProps {
  /** Admin: show who deleted the item. Defaults to false. */
  showDeletedBy?: boolean;
}

export function CampaignTrashButton({ showDeletedBy = false }: CampaignTrashButtonProps) {
  ...
  const items: TrashItem[] = deletedCampaigns.map((c: any) => ({
    ...
    ...(showDeletedBy ? { deletedByName: c.deletedByName } : {}),
  }));

...
interface DeleteCampaignsDialogBaseProps {
  open: boolean;
...
export function DeleteCampaignsDialogBase({
  open,
  onOpenChange,
  count,
  campaignNames,
  ...
  onConfirm,
  isPending,
}: DeleteCampaignsDialogBaseProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Delete {count} Campaign{count !== 1 ? "s" : ""}</DialogTitle>
          <DialogDescription>
            You are about to permanently delete {count} campaign{count !== 1 ? "s" : ""}. This action cannot be undone.
          </DialogDescription>
        </DialogHeader>
        ...
        {multiOrg && (
          <div className="flex items-start gap-3 rounded-md border border-yellow-300 bg-yellow-50 p-3 dark:border-yellow-700 dark:bg-yellow-950">
            ...
            <p className="text-sm text-yellow-800 dark:text-yellow-300">
              Selected campaigns belong to multiple organizations. Please select campaigns from a single organization.
            </p>
          </div>
        )}

        {!multiOrg && associatedEventCount > 0 && (
          <div className="flex items-start gap-3 rounded-md border border-border bg-muted/50 p-3">
            <Checkbox
              id="delete-events"
              checked={deleteEvents}
              onCheckedChange={(checked) => setDeleteEvents(!!checked)}
            />
            <label htmlFor="delete-events" className="text-sm cursor-pointer leading-tight">
              Also delete {associatedEventCount} associated event{associatedEventCount !== 1 ? "s" : ""}
            </label>
          </div>
        )}

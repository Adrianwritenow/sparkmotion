      ...
      {events.map((event) => {
        ...
        <div key={event.id} className="flex items-start gap-3">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted-foreground mb-4">
            <div className="flex items-center gap-2">
              ...
              <span>
                {event.startDate && event.endDate ? (
                  ...
                  `Starts ${new Date(event.startDate).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" })}`
                ) : (() => {
            ...
            {(event.venueName || event.city || event.formattedAddress || event.location) && (
              <div className="flex items-start gap-1.5">
                <MapPin className="w-4 h-4 mt-0.5 shrink-0" />
                <div className="text-sm">
                  {event.venueName && <span className="font-medium">{event.venueName}</span>}
                  {event.venueName && (event.city || event.formattedAddress || event.location) && <span className="mx-1">-</span>}
                  <span>
                    {event.city
